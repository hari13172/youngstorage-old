...
import AlertResponce from "@/component/alert";
...
function signin() {
  ...
  const onSubmit = async (data) => {
    setError((a) => ({ ...a, status: false, data: "" }));
    try {
      ...
      if (token) {
        ...
        setError((a) => ({
          ...a,
          status: true,
          data: "somthing went wrong contact admin",
        }));
      }
    } catch (error) {
      setError((a) => ({ ...a, status: true, data: error.message }));
    }
  };

  return (
    <div className="form-container">
      {error?.status && <AlertResponce data={error.data} />}
